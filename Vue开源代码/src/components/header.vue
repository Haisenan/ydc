                ...
                <div class="ydc-column-user">
                    ...
                    <div class="ydc-user-info">
                        <div class="ydc-user-info-name">
                            <a href="javascript:;">{{display_name}}</a>
                        </div>
                        <div class="ydc-user-info-func ydc-flex">
                            ...
                            <a href="javascript:;" @click="signOut()">{{state}}</a>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
  ...
  created(){
    if(localStorage.token != ''){
      this.state = '退出';
      this.display_name = sessionStorage.email;
      this.icon = ''+ sessionStorage.icon
      // 头像不建议去  session 拿 你也拿不到  只能是去后台发请求
      // 登陆通过之后  请求 用户名对用的 查询 表 icon字段  返回数据
    }
  },
  methods:{
    signOut(){
